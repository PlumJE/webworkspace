// 1) 함수 선언문 : 호이스팅, 중복 선언 가능 = var 방식
console.log(firstPlus);
console.log(firstPlus(10, 15));
...
// 2) 함수 표현식 : 선언 이후 사용, 중복 선언 불가 = let/const 방식 주로 const 사용
// console.log(secondFunc); // Error
// console.log(secondFunc(11, 22)); // Error
...
[10, 5, 23, 1].forEach(function(value, index, array) { // 함수를 매개 변수로 전달받는 forEach -포리치
...
function orderSet() {
    let side = '감자튀김';  // 원칙상 사라져야 함
    let beverage = '콜라';  // 원칙상 사라져야 함
    return function selectMenu(burger) {
        console.log('Set Menu : ', burger, side, beverage);
    }
}

const mcdonald = orderSet(); // 변수 선언으로 사라져야할 함수가 사라지지 않음
...
function User(id, pwd){ // 함수 이름을 대문자로 해야함 - 자바스크립트는 대문자를 생성자 함수로 인식함
    // 필드
    this.id = id;   // 키워드는 this 기반으로 만듬
    this.pwd = pwd; // this는 생성자 함수 객체를 가리킴 new 연산자를 통해 호출해야 의미있음
...
console.clear();
[10, 5, 23, 1].forEach((value, index, array) => {
    console.log(`${index} : ${value}`);
});
