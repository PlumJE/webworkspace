...
function addBody(list) {
    ...
    $.each(list, function(idx, info) {
        ...
        trTag.on('click', function(event) {
            ...
            // jQuery방식
            // let selectId = $(selectTrTag).children().eq(1).text();
            findUserById(info.id);
        });
        ...
        // 4) 가입일자
        tdTag = $('<td/>');
        tdTag.text(setDateFormat(info.joinDate));
        trTag.append(tdTag);
...
function findUserById(userId) {
    // 서버가 가지고 있던 회원들 중 지정한 항목으로 검색
    // GET, http://192.168.0.11:8099/userInfo?id=
...
function formUserInfo() {
    // 현재 입력된 회원정보 확인 : 모든 입력태그의 value를 확인
...
function updateUserInfo(event) {
    ...
    // 2) 서버에 전송 : AJAX
    $.ajax('http://192.168.0.11:8099/userUpdate?id=' + userInfo.id, {
        method: 'POST',
